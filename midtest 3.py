...
def select_all(event=None):
    ...
    return 'break' # 이벤트가 다른 바인딩으로 전파되는 것을 막습니다.
...
#    - undo=True: 실행 취소/다시 실행 기능 활성화
text_area = tk.Text(window, wrap='word', undo=True)

# 편집 메뉴 항목 추가
edit_menu.add_command(label="실행 취소", command=text_area.edit_undo)
edit_menu.add_command(label="다시 실행", command=text_area.edit_redo)
